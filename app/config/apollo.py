...
apollo_client = ApolloClient(app_id=APOLLO['APP_ID'],
                             portal_server_url=APOLLO['PORTAL_SERVER_URL'],
                             config_server_url=APOLLO['CONFIG_SERVER_URL'],
                             authorization=APOLLO['AUTHORIZATION'],
                             env=APOLLO['ENV'],
                             cycle_time=APOLLO['CYCLE_TIME'])


class _RagEsConfig(object):
    def __init__(self):
        self._es_index_config_key = "es.index.config"

    def _load_es_config(self):
        return json.loads(apollo_client.get_value(self._es_index_config_key))
